...
export function specToMarkdown(spec: TechnicalSpec): string {
  ...
  if (spec.considerations && spec.considerations.length) {
  ...
  if (spec.steps && spec.steps.length) {
    ...
    for (const s of spec.steps) {
      parts.push(`${s.order}. **${s.title}** — creates: ${s.artifacts.map((n) => `\`${n}\``).join(', ')}`);
      if (s.capituDevCall) {
  ...
  parts.push('---');
  parts.push('_Generated by capitu-spec. Spec is a proposal — validate against tenant via `capituSpecValidate` before implementing._');
  return parts.join('\n');
}
