...
import { globalDesignRationale, rationaleFor } from '../rationale.js';
...
const artifactSchema = z.object({
  ...
  /**
   * service-binding only: name of the SRVD the binding exposes. Required for
   * service-binding artifacts. Ignored for other kinds.
   */
  serviceDefinition: z.string().optional(),
  /**
   * service-binding only: LLM-friendly binding label (e.g. "ODataV4-UI",
   * "OData V2 Web API"). Defaults to "ODataV4-UI" if omitted.
   */
...
export interface ProposedArtifact {
  ...
  /** service-binding only: target SRVD name (passed verbatim to ADT). */
  serviceDefinition?: string;
  /** service-binding only: LLM-friendly binding label. */
  bindingType?: string;
}
...
async function buildProposedArtifacts(
  ...
  for (const a of input.artifacts) {
    ...
    artifacts.push({
      ...
      fieldResolution,
      serviceDefinition: a.serviceDefinition?.toUpperCase(),
  ...
  for (const a of artifacts) {
    if (a.kind === 'service-binding' && !a.serviceDefinition) {
      blockingErrors.push(
        `Service binding "${a.name}" is missing required field "serviceDefinition" — set it to the SRVD name this binding exposes.`,
      );
    }
  }
  ...
  if (hasSrvb) {
    warnings.push(
      'Service bindings (SRVB) are now created via raw ADT XML — no Eclipse wizard required. Defaults: ODataV4-UI, version 0001. Override via "bindingType".',
    );
  }
...
function composeProposalMarkdown(
  ...
  for (const a of artifacts) {
    ...
    if (a.sourceProvided) {
      ...
      lines.push(
        '',
        `_Source generated by \`${a.generator}\`${a.hasPlaceholders ? ' — contains <PLACEHOLDERS> that must be reviewed before apply' : ''}._`,
      );
    } else {
