...
export async function main(): Promise<void> {
  ...
  try {
    ...
    // stderr is the only channel the MCP client sees for diagnostics.
    process.stderr.write(
      `[capitu-docs] startup failed: ${err instanceof Error ? err.message : String(err)}\n`,
    );
    process.exit(1);
  }

  const server = new Server(
    { name: 'capitu-docs', version: VERSION },
    { capabilities: { tools: {} } },
  );
  ...
  server.setRequestHandler(CallToolRequestSchema, async (req) => {
    ...
    try {
      // The union over heterogeneous tool schemas defeats per-call inference;
      // runTool itself enforces Zod parsing at runtime, so the cast is safe.
  ...
  process.stderr.write(`[capitu-docs] v${VERSION} ready (${ALL_TOOLS.length} tools)\n`);
}
...
if (invokedDirectly) {
  main().catch((err) => {
    process.stderr.write(`[capitu-docs] fatal: ${err}\n`);
    process.exit(1);
  });
}
